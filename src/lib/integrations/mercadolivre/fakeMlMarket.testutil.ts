...
export class FakeMlMarket extends FakeMlApi {
  ...
  fetch = async (input: string, init?: RequestInit): Promise<Response> => {
    ...
    const isMarket = /^\/(items|user-products|sites|categories)\b/.test(path) || /^\/users\/\d+\/items\/search$/.test(path)
    ...
    if (method === 'GET' && path === '/categories/MLB1234/attributes') return json(200, this.categoryAttributes)
    ...
    if (method === 'POST' && path === '/items/validate') {
      ...
      if (errors.length === 0 && this.validateWarnings.length === 0) return new Response(null, { status: 204 })
      return json(400, {
        message: 'Validation error', error: 'validation_error', status: 400,
        cause: [...errors.map((e) => ({ ...e, type: 'error', department: 'items' })), ...this.validateWarnings.map((w) => ({ ...w, type: 'warning', department: 'items' }))],
      })
    }
  ...
  private itemErrors(payload: Record<string, unknown>): Array<{ code: string; message: string }> {
    ...
    const attrs = new Set(((payload.attributes as Array<{ id: string }>) ?? []).map((a) => a.id))
