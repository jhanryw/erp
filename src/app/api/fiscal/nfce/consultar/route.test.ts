...
// consultNfceStatus (sempre escopado por user.company_id).
import { describe, it, expect, vi, afterEach } from 'vitest'
import { POST } from './route'
import * as sessionModule from '@/lib/supabase/session'
import * as consultModule from '@/services/fiscal/consultNfceStatus'
...
function buildRequest(body: unknown): Request {
  return new Request('http://localhost/api/fiscal/nfce/consultar', {
    method: 'POST',
...
describe('POST /api/fiscal/nfce/consultar — acesso liberado pra qualquer usuário autenticado da empresa', () => {
  afterEach(() => { vi.restoreAllMocks() })

  it('role=usuario (seller) → passa pelo gate, company_id sempre da sessão', async () => {
    mockSession('usuario', 1)
    const consultSpy = vi.spyOn(consultModule, 'consultNfceStatus').mockResolvedValue({ ok: true, data: { status: 'authorized' } as any })
  ...
  it('role=admin → continua funcionando normalmente', async () => {
    mockSession('admin', 1)
    vi.spyOn(consultModule, 'consultNfceStatus').mockResolvedValue({ ok: true, data: { status: 'authorized' } as any })

    const res = await POST(buildRequest({ sale_id: 42 }))
    expect(res.status).toBe(200)
  })

  it('venda de outra empresa → consultNfceStatus devolve erro (escopo por company_id não encontra nada), rota nunca vaza sucesso', async () => {
    mockSession('usuario', 1)
    vi.spyOn(consultModule, 'consultNfceStatus').mockResolvedValue({ ok: false, error: 'Nenhuma tentativa de emissão de NFC-e encontrada pra esta venda.', status: 404 })

    const res = await POST(buildRequest({ sale_id: 999 }))
    expect(res.status).toBe(404)
  })

  it('sem sessão/role insuficiente → 403, nunca chama consultNfceStatus', async () => {
    ...
    const consultSpy = vi.spyOn(consultModule, 'consultNfceStatus')
