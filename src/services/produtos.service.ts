...
export async function getProductSnapshot(productId: number, companyId?: number | null): Promise<Record<string, unknown> | null> {
  const admin = createAdminClient()
  let query = admin.from('products').select('id, name, sku, base_cost, base_price, active, category_id, supplier_id, brand_id, origin, ncm, cest, origem, unidade_med, wholesale_enabled').eq('id', productId)
