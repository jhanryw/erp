-- =============================================================================
-- 202609231100_rpc_create_sale_kits.sql
--
-- KITS — venda atômica (Fase E).
--
-- rpc_create_sale: MESMA assinatura de 19 parâmetros
-- (202608311201_rpc_create_sale_wholesale_channel.sql) → CREATE OR REPLACE
-- seguro, sem overload novo, grants preservados (reaplicados abaixo por
-- clareza, idempotentes).
--
-- Produtos normais: caminho de execução IDÊNTICO ao vigente quando a venda
-- não contém nenhum kit (mesmos locks, mesma validação item a item, mesmas
-- mensagens, mesmos movimentos). Única diferença textual: o INSERT em
-- sale_items agora faz RETURNING id (sem efeito observável).
--
-- Venda com kit (v_has_kit):
--   1. Requisitos físicos agregados (fn_resolve_stock_requirements):
--      standard → ela mesma × qtd; kit → componentes × qtd_por_kit × qtd.
--      Um carrinho com KIT(2×A) + 1×A exige 3×A — validado junto.
--   2. Locks FOR UPDATE em ordem determinística (product_variation_id,
--      stock_location_id) sobre o conjunto FÍSICO — nunca sobre o kit, que
--      não tem saldo. Mesma ordem usada por vendas sem kit → PDV×kit e
--      kit×kit serializam na mesma linha sem deadlock por ordem invertida.
--   3. Validação de TODOS os requisitos ANTES de qualquer baixa. Qualquer
--      falta → RAISE → ROLLBACK completo (nada baixado, nada registrado).
--   4. Item COMERCIAL do kit vai para sale_items (SKU/qtd/preço do kit);
--      custo = SUM(custo do componente × qtd_por_kit) calculado AQUI (o
--      unit_cost do payload é ignorado para kit — mesma fonte de custo do
--      restante do sistema: cost_override da variação ?? base_cost do
--      produto; nunca a tabela legada `stock`).
--   5. Consumo físico dos componentes (_consume_kit_components): baixa em
--      stock_balances, stock_movements por componente e snapshot em
--      sale_item_components (composição + custo + local de origem).
-- =============================================================================


-- ─── Requisitos físicos de um conjunto de itens ─────────────────────────────
-- Única definição SQL de "o que precisa sair do estoque para vender isto".
-- Espelhada em TS por resolveStockRequirements (src/lib/inventory/
-- stockRequirements.ts) — mesma regra, coberta por teste nos dois lados.
CREATE OR REPLACE FUNCTION public.fn_resolve_stock_requirements(p_items jsonb)
RETURNS TABLE (product_variation_id int, quantity int)
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  WITH items AS (
    SELECT (value->>'product_variation_id')::int AS pvid,
           (value->>'quantity')::int             AS qty
    FROM jsonb_array_elements(p_items)
  ),
  expanded AS (
    SELECT i.pvid AS pvid, i.qty AS qty
    FROM items i
    JOIN product_variations pv ON pv.id = i.pvid
    JOIN products p            ON p.id  = pv.product_id
    WHERE p.product_kind <> 'kit'
    UNION ALL
    SELECT kc.component_product_variation_id, i.qty * kc.quantity
    FROM items i
    JOIN product_variations pv      ON pv.id = i.pvid
    JOIN products p                 ON p.id  = pv.product_id
    JOIN product_kit_components kc  ON kc.kit_product_variation_id = i.pvid
    WHERE p.product_kind = 'kit'
  )
  SELECT pvid, SUM(qty)::int
  FROM expanded
  GROUP BY pvid
  ORDER BY pvid;
$$;

REVOKE ALL ON FUNCTION public.fn_resolve_stock_requirements(jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.fn_resolve_stock_requirements(jsonb) TO service_role;


-- ─── Consumo físico de UM item de kit ───────────────────────────────────────
-- Chamado só de dentro de rpc_create_sale (já com locks adquiridos e
-- requisitos validados). Revalida cada componente mesmo assim — defesa em
-- profundidade, nunca gera saldo negativo.
CREATE OR REPLACE FUNCTION public._consume_kit_components(
  p_company_id     int,
  p_sale_id        int,
  p_sale_item_id   int,
  p_kit_pvid       int,
...
DECLARE
  ...
  v_kit_sku    text;
BEGIN
  SELECT sku_variation INTO v_kit_sku FROM product_variations WHERE id = p_kit_pvid;

  FOR v_comp IN
    SELECT kc.component_product_variation_id AS pvid,
           kc.quantity                       AS qpk,
           cpv.product_id                    AS product_id,
           cpv.sku_variation                 AS sku,
           ROUND(COALESCE(cpv.cost_override, cp.base_cost, 0), 2) AS unit_cost
    ...
    WHERE kc.kit_product_variation_id = p_kit_pvid
      AND kc.company_id = p_company_id
      AND cp.company_id = p_company_id
    ORDER BY kc.component_product_variation_id
  LOOP
    v_need := v_comp.qpk * p_kit_qty;

    IF p_stock_mode = 'main_store' THEN
      SELECT quantity INTO v_cur
      FROM stock_balances
      WHERE product_variation_id = v_comp.pvid
        AND stock_location_id    = p_main_store_id
      FOR UPDATE;

      IF COALESCE(v_cur, 0) < v_need THEN
        RAISE EXCEPTION
          'Componente % do kit % sem saldo no Estoque Loja. Disponível: %, necessário: %.',
          v_comp.sku, v_kit_sku, COALESCE(v_cur, 0), v_need
          USING ERRCODE = 'P0001';
      END IF;

      UPDATE stock_balances
      SET quantity = quantity - v_need, last_updated = NOW()
      WHERE product_variation_id = v_comp.pvid
        AND stock_location_id    = p_main_store_id;
      ...
      INSERT INTO stock_movements (
        ...
        source_location_id, movement_type, reference_type, created_by, notes
      )
      VALUES (
        v_comp.pvid, v_comp.product_id, 'sale', -v_need,
        v_cur, v_cur - v_need, v_comp.unit_cost, p_sale_id::text, p_company_id,
        p_main_store_id, 'sale', 'sale', p_user_id,
        'Componente do kit ' || v_kit_sku
      );

      INSERT INTO sale_item_components (
        company_id, sale_id, sale_item_id, kit_product_variation_id,
        component_product_variation_id, stock_location_id,
        quantity_per_kit, kit_quantity, total_quantity, quantity, unit_cost
      )
      VALUES (
        p_company_id, p_sale_id, p_sale_item_id, p_kit_pvid,
        v_comp.pvid, p_main_store_id,
        v_comp.qpk, p_kit_qty, v_need, v_need, v_comp.unit_cost
      );

    ELSE
      SELECT COALESCE(SUM(sb.quantity), 0) INTO v_total
      FROM stock_balances sb
      JOIN stock_locations sl ON sl.id = sb.stock_location_id
      WHERE sb.product_variation_id = v_comp.pvid
        AND sl.company_id = p_company_id
        AND sl.active     = true;

      IF v_total < v_need THEN
        RAISE EXCEPTION
          'Componente % do kit % com estoque total insuficiente para venda online. Disponível: %, necessário: %.',
          v_comp.sku, v_kit_sku, v_total, v_need
          USING ERRCODE = 'P0001';
      END IF;
      ...
      FOR v_loc IN
        ...
        JOIN stock_balances sb ON sb.stock_location_id = sl.id
                              AND sb.product_variation_id = v_comp.pvid
      ...
      LOOP
        ...
        UPDATE stock_balances
        SET quantity = quantity - v_debit, last_updated = NOW()
        WHERE product_variation_id = v_comp.pvid
          AND stock_location_id    = v_loc.location_id;
        ...
        VALUES (
          v_comp.pvid, v_comp.product_id, 'sale', -v_debit,
          v_loc_qty, v_loc_qty - v_debit, v_comp.unit_cost, p_sale_id::text, p_company_id,
          v_loc.location_id, 'sale', 'online_order', p_user_id,
          'Componente do kit ' || v_kit_sku
        );
        ...
        VALUES (
          p_company_id, p_sale_id, p_sale_item_id, p_kit_pvid,
          v_comp.pvid, v_loc.location_id,
          v_comp.qpk, p_kit_qty, v_need, v_debit, v_comp.unit_cost
        );
      END LOOP;

      IF v_remaining > 0 THEN
        RAISE EXCEPTION
          'Erro interno: não foi possível debitar % unidades restantes do componente % (kit %).',
          v_remaining, v_comp.sku, v_kit_sku
          USING ERRCODE = 'P0001';
      END IF;
    END IF;
  END LOOP;
...
REVOKE ALL ON FUNCTION public._consume_kit_components(int, int, int, int, int, text, int, uuid) FROM PUBLIC, anon, authenticated;
