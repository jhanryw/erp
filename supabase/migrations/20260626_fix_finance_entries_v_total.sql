-- =============================================================================
-- 20260626_fix_finance_entries_v_total.sql
--
-- PROBLEMA: rpc_create_sale (desde 20260617) registrava v_gross em finance_entries
--   e adicionava segunda entrada income/cashback_used.
--   Resultado: venda R$100 paga 100% com crédito = R$200 de receita registrada.
--
-- REGRA CORRETA:
--   finance_entries registra apenas v_total (valor efetivamente pago pelo cliente).
--   v_total = v_gross - cashback_usado
--   Se v_total = 0 (100% pago com crédito), não cria entrada de receita.
--   Crédito usado rastreado em sales.cashback_used — não duplicar em finance_entries.
--
-- EXEMPLOS:
--   Venda R$100 sem crédito:        income/sale = 100
--   Venda R$100, R$30 crédito:      income/sale = 70
--   Venda R$100, R$100 crédito:     sem entrada (v_total = 0)
--
-- BASE: 20260617_rpc_create_sale_stock_mode.sql (versão atual em produção).
-- Única alteração: bloco finance_entries (linhas 331-349 do original).
-- Todo o resto é idêntico ao 20260617.
-- =============================================================================
...
BEGIN
  ...
  -- ─── Finance entries ─────────────────────────────────────────────────────────
  -- Registra apenas o valor efetivamente pago pelo cliente (v_total).
  -- v_total = v_gross - cashback_usado.
  -- Se v_total = 0 (100% pago com crédito), não gera entrada de receita.
  IF v_total > 0 THEN
