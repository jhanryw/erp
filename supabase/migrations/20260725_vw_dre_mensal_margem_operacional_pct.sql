-- =============================================================================
-- 20260725_vw_dre_mensal_margem_operacional_pct.sql
--
-- Financeiro 2.1 — correção pontual: margem_operacional_pct na vw_dre_mensal
--
-- Recria a view v3 (20260724_vw_dre_mensal_v3_revenue_reversal.sql) com uma
-- única coluna adicional: margem_operacional_pct = resultado_operacional /
-- receita_liquida * 100, arredondada a 2 casas, mesma convenção de
-- margem_bruta_pct e margem_liquida_pct. Nenhuma outra fórmula ou coluna
-- muda — CTEs cmv_por_venda, vendas_original, vendas_reversao, vendas,
-- lancamentos, base e calculado são idênticas à migration anterior.
--
-- Motivo: a página /financeiro/dre não deve calcular percentual algum — o
-- badge de "Resultado Operacional" precisa vir pronto da view, não ser
-- recalculado em JavaScript.
-- =============================================================================
...
COMMENT ON VIEW public.vw_dre_mensal IS
  ...
  'percentual da DRE — a página não recalcula nenhum. Atualizado em 20260725.';
...
/*
DROP VIEW IF EXISTS public.vw_dre_mensal;
-- Reaplicar o CREATE VIEW de 20260724_vw_dre_mensal_v3_revenue_reversal.sql
-- na íntegra, incluindo seu COMMENT ON VIEW original. Depois:
-- GRANT SELECT ON public.vw_dre_mensal TO authenticated, service_role;
-- NOTIFY pgrst, 'reload schema';
*/
