-- =============================================================================
-- 20260612_remove_transfer_requirement.sql
--
-- Remove a obrigatoriedade de transferência antes de vender.
--
-- Antes: rpc_create_sale exigia saldo no Estoque Loja (is_main_store=true).
--        Se o produto estivesse em outro local, bloqueava com "Transfira antes de vender".
--
-- Depois: debita do primeiro local que tiver saldo suficiente.
--         Se não houver saldo em nenhum local, aí sim bloqueia.
-- =============================================================================
...
DECLARE
  ...
  v_brazil_date     date;
  v_main_store_id   int;
  v_debit_location  int;   -- local de onde vai debitar
...
BEGIN
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_debit_location IS NULL THEN
      -- Verifica total geral para mensagem mais clara
