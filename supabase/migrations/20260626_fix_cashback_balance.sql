-- =============================================================================
-- 20260626_fix_cashback_balance.sql
--
-- PROBLEMA: v_cashback_balance.available_balance nunca decresce quando crédito
--   é usado. A view soma apenas earn(status='available') e ignora o consumo.
--   Resultado: cliente pode usar o mesmo crédito infinitas vezes.
--
-- CORREÇÃO — Opção B (mínima mudança de schema):
--
--   1. v_cashback_balance: available_balance = earn_available - use
--      total_used já existia na view mas nunca era preenchido porque nenhum
--      código inseria type='use'. Agora será preenchido.
--
--   2. rpc_create_sale:
--      a) Se p_cashback_used > 0, valida que available_balance >= v_eff_cashback.
--      b) Após criar a venda, insere cashback_transactions(type='use').
--
-- NÃO ALTERADO:
--   - rpc_cancel_sale / rpc_return_sale — já inserem earn compensatório
--   - rpc_process_exchange — já credita via earn(available)
--   - Geração de earn em venda normal — permanece desligada (sem cashback_config)
--   - finance_entries — usa v_total (do 20260626_fix_finance_entries_v_total.sql)
--
-- CENÁRIOS:
--   Cliente com R$100 crédito:
--     Usa R$40 → available = 100 - 40 = 60 ✓
--     Tenta usar R$110 → RAISE EXCEPTION (saldo insuficiente) ✓
--     Cancela venda onde usou R$40 → earn +40 inserido → available = 60 + 40 = 100 ✓
-- =============================================================================


-- ─── 1. Corrigir view v_cashback_balance ─────────────────────────────────────
...
SELECT
  ...
  GREATEST(0,
    SUM(CASE WHEN type = 'earn' AND status = 'available' THEN amount ELSE 0 END) -
...
-- ─── 2. rpc_create_sale com validação e consumo de crédito ───────────────────
-- Base: 20260626_fix_finance_entries_v_total.sql (15 parâmetros, finance = v_total).
-- Adições: v_avail_credit, validação de saldo, INSERT type='use'.
...
DECLARE
  ...
  v_avail_credit    numeric;        -- saldo real do cliente (earn - use)
  v_item            jsonb;
  v_pvid            int;
  v_qty             int;
...
BEGIN
  ...
  -- Feito após calcular v_eff_cashback (que pode ser menor que p_cashback_used).
  -- Computa diretamente da tabela (não da view) para pegar valor mais recente.
  ...
  -- Inserido logo após a venda para ter o sale_id disponível.
  -- O tipo 'use' é subtraído pelo v_cashback_balance.available_balance.
  -- Em cancelamento/devolução, rpc_cancel_sale/rpc_return_sale já inserem um
  -- novo earn(available) para compensar — sem necessidade de alterar o 'use'.
  ...
  -- ─── Finance entries ──────────────────────────────────────────────────────
  -- Registra apenas v_total (o que o cliente efetivamente pagou em dinheiro/pix/cartão).
  -- Crédito usado reduz v_total — não gera entrada separada de receita.
  IF v_total > 0 THEN
