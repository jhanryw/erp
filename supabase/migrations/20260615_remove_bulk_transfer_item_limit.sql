-- =============================================================================
-- 20260615_remove_bulk_transfer_item_limit.sql
--
-- Remove o limite máximo de 200 itens por batch da rpc_stock_transfer_bulk.
--
-- Trecho removido em relação a 20260615_rpc_stock_transfer_bulk.sql:
--
...
--   END IF;
--
-- Tudo o mais permanece idêntico à versão anterior:
--   - validate-all-first
--   - erros detalhados por item acumulados antes de qualquer escrita
--   - rejeição de product_variation_id duplicado no batch
--   - validação origem ≠ destino
--   - validação de locais ativos e mesma empresa
--   - validação de saldo suficiente na origem
--   - custo médio ponderado no destino
--   - batch_id único compartilhado por todos os stock_movements do lote
--   - locks determinísticos anti-deadlock
--   - re-validação de saldo pós-lock (proteção contra race condition)
--
-- A variável v_item_count é mantida: ainda é usada para validar lista vazia (1e).
--
-- O que esta migration NÃO faz:
--   - Não altera a migration anterior (20260615_rpc_stock_transfer_bulk.sql)
...
BEGIN
  ...
  -- 1f. (removido) Limite máximo de 200 itens — sem restrição de tamanho de lote.
...
-- 1. Confirmar que o texto "200" não aparece no corpo da função
--    SELECT prosrc
--    FROM pg_proc
--    WHERE proname = 'rpc_stock_transfer_bulk';
--    → o resultado NÃO deve conter "200" nem "Limite máximo"

-- 2. Confirmar que lotes grandes passam pela validação (use pvids com saldo real)
--    SELECT rpc_stock_transfer_bulk(
--      <from_id>, <to_id>, 'teste sem limite', auth.uid(),
...
--         FROM stock_balances
--         WHERE stock_location_id = <from_id> AND quantity >= 1
--         LIMIT 201
--       ) sub)
--    );
--    → não deve retornar erro de limite; deve validar cada item individualmente

-- 3. Confirmar que lista vazia ainda retorna erro
--    SELECT rpc_stock_transfer_bulk(<from_id>, <to_id>, 'teste', auth.uid(), '[]'::jsonb);
--    → { ok: false, errors: [{ reason: "Nenhum item informado." }] }

-- 4. Confirmar que origem = destino ainda retorna erro
--    SELECT rpc_stock_transfer_bulk(1, 1, 'teste', auth.uid(), '[{"product_variation_id":1,"quantity":1}]'::jsonb);
--    → { ok: false, errors: [{ reason: "Origem e destino não podem ser iguais." }] }

-- 5. Confirmar que variação duplicada ainda retorna erro
--    SELECT rpc_stock_transfer_bulk(
--      <from_id>, <to_id>, 'teste', auth.uid(),
--      '[{"product_variation_id":<pvid>,"quantity":1},{"product_variation_id":<pvid>,"quantity":2}]'::jsonb
--    );
--    → { ok: false, errors: [{ reason: "Variação duplicada no lote." }] }
-- =============================================================================
